...
<div class="fuente">
    ...
    <div>
        <table class="table table-bordered table-general" style="width:100%">
            ...
            <tbody>
                @foreach ($datosNomina as $nom)
                @if (isset($nom->unidades))
                    <tr>
                        <td width="15%" class="td">{{ $nom->folio }}</td>
                        <td width="20%" class="td">{{ $nom->descripcion }}</td>
                        <td width="10%" class="td centrado">{{ $nom->fecha_creacion }}</td>
                        @if (count( $nom->fecha_cierre ) >= 1)
                            <td width="10%" class="td centrado">{{ $nom->fecha_cierre[0]->fecha_cerrado }}</td>    
                        @else
