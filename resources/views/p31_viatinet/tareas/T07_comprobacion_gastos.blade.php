...
@section('contenido')
    ...
    <form id="form_finalizar_tarea" action="{{ route('viatinet.comprobacion.gastos', [$solicitudViatico, $instanciaTarea]) }}" method="POST">
        @method('post')
        @csrf
        <div class="card card-custom mt-8">
            <div class="card-header">
                <div class="card-title">
                    <h3 class="card-label">Respuesta</h3>
                </div>
            </div>
            <div class="card-body">
                @if ($solicitudViatico->estatus == "COMPLETADO")
                    <div class="alert alert-custom alert-outline-2x alert-outline-success fade show" role="alert">
                        <div class="alert-icon"><i class="fas fa-check"></i></div>
                        <div class="alert-text">SOLICITUD AUTORIZADA</div>
                    </div>
                    @php
                        $archivoAutorizacion = $solicitudViatico->documentos()
                            ->whereHas("tipoDocumento", function($query) {
                                $query->where("identificador", "documento_autorizacion");
                            })
                            ->with("tipoDocumento")
                            ->first();
                    @endphp
                    <a
                        ...
                        href="{{ $archivoAutorizacion->ruta_download }}">
                        <i class="fas fa-file-download"></i> Descargar autorización
                    </a>
                @elseif ($solicitudViatico->estatus == "RECHAZADO")
                    <div class="alert alert-custom alert-outline-2x alert-outline-danger fade show mb-0" role="alert">
                        <div class="alert-icon"><i class="fas fa-times"></i></div>
                        <div class="alert-text">SOLICITUD RECHAZADA: <br> {{ $solicitudViatico->motivo_rechazo }}</div>
                    </div>
                @endif
            </div>
