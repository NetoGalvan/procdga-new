...
@section('title', 'REIMPRESIÓN DE LAYOUT PREMIO PUNTUALIDAD Y ASISTENCIA')
...
@section('subheader')
    @include('layouts.partials.main.subheader', ["subheader" => [
        ...
        "breadcrumbs" => [
            ...
            2 => [
                'activo' => true,
                'titulo' => 'REIMPRESIÓN DE LAYOUT PREMIO PUNTUALIDAD Y ASISTENCIA'
            ]
        ]
    ]])
@endsection
...
@section('contenido')
    ...
    <form action="{{ route('reporte.reimpresion.layout.premio') }}" method="POST" id="reporte_reimpresion_layout">
        @method('post')
        @csrf
                ...
                <div class="alert alert-custom alert-success" role="alert">
                    ...
                    <div class="alert-text">
                        <strong>
                            ...
                            <ul class="mb-2">
                                <li>Ingrese el folio para la reimpresión de layout premio de puntualidad y asistencia</li>
                            </ul>
                        </strong>
                    </div>
                </div>
...
@push('scripts')
    <script src="{{ asset('js/p20_premio_puntualidad_asistencia/reportes/reporteReimpresionLayout.js') }}"></script>
    <script src="{{ asset('metronic/plugins/custom/bootstraptable/js/bootstrap-table.min.js') }}"></script>
    <script src="{{ asset('metronic/plugins/custom/bootstraptable/js/bootstrap-table-es-SP.js') }}"></script>
    <script>
        let urlReporteReimpresionLayoutBuscar = @json( route('reporte.reimpresion.layout.premio.buscar' ) );
        let urlReimpresionLayout = @json( route('descargar.excel.reimpresion.layout.premio' ) );
    </script>
@endpush
