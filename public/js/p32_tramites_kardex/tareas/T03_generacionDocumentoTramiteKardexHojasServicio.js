...
$(document).ready(function () {
    ...
    /*Evento para mostrar u ocultar motivo de baja con radio button */
    $('#grupo_motivo_baja').hide();
    $("input[id$='detalle_baja']").click(function() {
        $('#grupo_motivo_baja').show();
    });

    $("input[id$='detalle_aportacion']").click(function() {
        $('#grupo_motivo_baja').hide();
    });
});
...
function periodoFormatter(value, row) {
    return `DESDE: ${row.fecha_desde} </br> HASTA: ${row.fecha_hasta}`;
}

function puestoFormatter(value, row) {
    return `PUESTO: ${row.puesto} </br>
            CÓDIGO: ${row.codigo_puesto} </br>
            NIVEL: ${row.nivel_salarial}`;
}
